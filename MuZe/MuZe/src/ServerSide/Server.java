...
public class Server extends JFrame implements ActionListener {
    ...
    while(true)
      {
        ...
        else if (request_type == ISFINISHED)
        {
            if (currentlystreaming)
            {
                theServer.send_RTSP_negative_response();
            }
            else
            {
                theServer.send_RTSP_response();
  ...
  private int parse_RTSP_request()
  {
    ...
    try{
      ...
      else if ((new String(request_type_string)).compareTo("ISFINISHED") == 0)
          request_type = ISFINISHED;
      if (request_type == SETUP)
        {
  ...
  private void send_RTSP_negative_response()
  {
      try {
          RTSPBufferedWriter.write("RTSP/1.0 300 OK"+CRLF);             //tree hunnit is for negative
          RTSPBufferedWriter.write("CSeq: "+RTSPSeqNb+CRLF);
          RTSPBufferedWriter.write("Session: "+RTSP_ID+CRLF);
          RTSPBufferedWriter.flush();
